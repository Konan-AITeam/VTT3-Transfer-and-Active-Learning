...
if __name__ == '__main__':
    ...
    cfg.imdbval_name_list = univ_info(
        cfg.datasets_list, 'imdbval_name', test=True)
    cfg.train_scales_list = univ_info(cfg.datasets_list, 'SCALES', test=True)
    cfg.test_scales_list = univ_info(
        cfg.datasets_list, 'SCALES_TEST', test=True)
    cfg.train_rpn_batchsize_list = univ_info(
        cfg.datasets_list, 'RPN_BATCHSIZE', test=True)
    cfg.train_batchsize_list = univ_info(
        cfg.datasets_list, 'BATCH_SIZE', test=True)
    cfg.rpn_positive_overlap_list = univ_info(
        cfg.datasets_list, 'RPN_POSITIVE_OVERLAP', test=True)
    cfg.rpn_nms_thresh_list = univ_info(
        cfg.datasets_list, 'RPN_NMS_THRESH', test=True)
    cfg.train_fg_thresh_list = univ_info(
        cfg.datasets_list, 'FG_THRESH', test=True)
    cfg.MAX_NUM_GT_BOXES_LIST = univ_info(
        cfg.datasets_list, 'MAX_NUM_GT_BOXES', test=True)
    cfg.ANCHOR_SCALES_LIST = univ_info(
        cfg.datasets_list, 'ANCHOR_SCALES', test=True)
    cfg.ANCHOR_RATIOS_LIST = univ_info(
        cfg.datasets_list, 'ANCHOR_RATIOS', test=True)
    ...
    for i in range(len(cfg.num_classes)):
        cfg.ANCHOR_NUM[i] = len(cfg.ANCHOR_SCALES_LIST[i]) * \
            len(cfg.ANCHOR_RATIOS_LIST[i])

    cfg.rpn_univ = args.rpn_univ == 'True'
    if cfg.rpn_univ:
        cfg.ANCHOR_SCALES_LIST = [cfg.ANCHOR_SCALES]
        cfg.ANCHOR_RATIOS_LIST = [cfg.ANCHOR_RATIOS]
        cfg.ANCHOR_NUM = [len(cfg.ANCHOR_SCALES_LIST[0])
                          * len(cfg.ANCHOR_RATIOS_LIST[0])]
    cfg.mode = 'universal'
...
#   pprint.pprint(cfg)
    ...
    if cfg.use_mux:
        print("INFO: Using BN MUX")
    else:
        print("INFO: Not using BN MUX")

    imdb, roidb, ratio_list, ratio_index = combined_roidb(
        cfg.imdbval_name_list[cls_ind], False)
    ...
    for i in range(num_images):
        ...
        rois, cls_prob, bbox_pred, \
            ...
            rois_label = fasterRCNN(
                im_data, im_info, gt_boxes, num_boxes, cls_ind)
